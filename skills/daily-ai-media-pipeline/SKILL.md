...
description: Use when working inside the ClawMax project to run or design the daily AI media workflow that creates a technical report first and then a WeChat draft for human review.
version: 1.0.0
...
This is a project-specific skill for the ClawMax AI media workflow. It defines the daily content-production chain that turns AI frontier information into a professional technical report and then into a WeChat public-account draft.

The pipeline should stay simple in the first stage: file-based handoff, traceable sources, clear output directories, and human review before publication.
...
- The workflow should produce files under `reports/YYYY-MM-DD/` and `articles/YYYY-MM-DD/`.
...
6. Verify that the draft contains required sections and human review notes.
7. Summarize outputs and next human action.
8. If the process exposed a reusable improvement, update the relevant project skill.
...
- the final output remains draft-only;
- the user can review and publish manually.

## Common Pitfalls

1. Automating too early. Fix by stabilizing mock-source and manual runs before scheduling.
2. Treating the WeChat article as the source of truth. Fix by keeping the technical report as the factual base.
3. Overwriting human edits during reruns. Fix by checking existing files and using versioned filenames.
